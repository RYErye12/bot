...
class SecondRegistrationModal(discord.ui.Modal, title="Step 2: Additional Information"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not data:
            await interaction.response.send_message(
                "You must complete Step 1 first!", ephemeral=True
            )
            return
        ...
        try:
            ...
            # Get the role IDs
            role_to_add = interaction.guild.get_role(1313488099426308156)  # Role to add
            role_to_remove = interaction.guild.get_role(1314245202990596187)  # Role to remove
            role_to_remove2 = interaction.guild.get_role(1315700246524723292)  # Role to remove

            if role_to_add:
                await interaction.user.add_roles(role_to_add)
            if role_to_remove:
                await interaction.user.remove_roles(role_to_remove)
                await interaction.user.remove_roles(role_to_remove2)

            await interaction.response.send_message("Your details have been successfully updated.", ephemeral=True)
...
class RegistrationCog(commands.Cog, name="registration"):
    ...
    @app_commands.command(name="list_users", description="List all registered users.")
    ...
    async def indb(self, interaction: discord.Interaction, user: discord.User = None):
        ...
        try:
            ...
            if row is None:
                await interaction.response.send_message(f"No data found for {user.name}#{user.discriminator}.", ephemeral=True)
                return
